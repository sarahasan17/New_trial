...
def training(total_words,max_sequence_len,predictors,label):
	...
	model.add(Dense(int((total_words + 1) / 2), activation='relu', kernel_regularizer=regularizers.l2(0.01)))
	model.add(Dense(total_words+1, activation='softmax'))
	model.compile(loss='categorical_crossentropy',
			optimizer='adam', metrics=['accuracy'])
	model.fit(predictors, label, epochs=1, batch_size=32, verbose=1)
	
	return model
total_words,max_sequence_len,predictors,label=preprocess(data)
model=training(total_words,max_sequence_len,predictors,label)
